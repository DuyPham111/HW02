...
| **B013** | [FR-08 — ngoài phạm vi] Ô "Tổng tiền thanh toán" cho phép chỉnh sửa tự do, gửi thẳng giá trị giả lên server | **Pre:** Đăng nhập, giỏ có sản phẩm (giỏ = 350.000).<br>**Steps:** 1. Vào Checkout. 2. Sửa ô "Tổng tiền thanh toán" từ 350000 thành giá trị bất kỳ (vd 1000). 3. Bấm "Xác Nhận Thanh Toán". 4. Kiểm tra đơn hàng vừa tạo (lịch sử đơn hàng) xem `total_amount` lưu là bao nhiêu.<br>**Expected:** Ô phải chỉ đọc (`readOnly`); backend phải tự tính lại tổng tiền từ giỏ hàng, không nhận `total_amount` client gửi (spec FR-08).<br>**Actual:** Ô nhập tự do (`<input type="number">` không `readOnly`, `Checkout.jsx:93-102`), giá trị này được gửi thẳng làm `total_amount` khi checkout (`Checkout.jsx:43-47`) — **[cần xác nhận thêm: đơn hàng tạo ra có lưu đúng giá trị đã sửa hay không — xem bước xác nhận bổ sung]**. | FR-08 (ngoài 4 feature đã chọn) | `FR-08_bugs/B013.png` | [link] | Open |
| **B014** | [FR-08 — ngoài phạm vi] Giỏ hàng không được xóa sau khi thanh toán thành công | **Pre:** Đăng nhập, giỏ có sản phẩm.<br>**Steps:** 1. Vào Checkout, hoàn tất thanh toán (bấm "Xác Nhận Thanh Toán", thấy "Thanh toán thành công!"). 2. Quay lại trang Giỏ hàng.<br>**Expected:** Giỏ hàng trống (spec FR-08: "Sau thanh toán thành công, giỏ hàng được xóa").<br>**Actual:** Giỏ hàng vẫn còn nguyên sản phẩm vừa thanh toán. Khớp code: `clearCart` được lấy ra (`Checkout.jsx:8`) nhưng không được gọi ở đâu trong `handleCheckout`. | FR-08 (ngoài 4 feature đã chọn) | `FR-08_bugs/B014.png` | [link] | Open |
