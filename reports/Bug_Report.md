...
| **B006** | [FR-09] Đơn đúng bằng ngưỡng tối thiểu bị từ chối (off-by-one `>`) | **Pre:** User đăng nhập, giỏ = đúng 300.000 (`TEST-300000`).<br>**Steps:** 1. Vào Checkout. 2. Nhập `SAVE10`, Áp dụng.<br>**Expected:** Chấp nhận, giảm 30.000, còn 270.000 (spec C3 `>=`).<br>**Actual:** Bị từ chối: "Đơn hàng chưa đủ giá trị tối thiểu 300.000 ₫ để áp dụng mã này" — đơn hàng **đúng bằng** ngưỡng vẫn bị coi là chưa đủ. Khớp chính xác với code `if (total_amount > coupon.min_order_amount)` (`server.js:379`) dùng `>` thay vì `>=` theo spec. So sánh: giỏ 299.999 (dưới ngưỡng 1đ) và giỏ 300.000 (đúng ngưỡng) cho **cùng một kết quả từ chối** — chứng minh rõ off-by-one. | FR-09 | `FR-09_bugs/B006.png` | [link] | Open |
