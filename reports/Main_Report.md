...
| FR02M-DT-06 | Không chặn submit khi để trống email/mật khẩu, thiếu dấu `*` bắt buộc | B019 | `FR-02-mobile_bugs/B019.png` |
...
| Designed | Executed | Pass | Fail | Not run | Bugs |
|----------|----------|------|------|---------|------|
| 10 (6 Domain + 4 BVA cross-check) | 9 | 4 | 5 | 1 (BV-04, bỏ qua có chủ đích) | 3 (B001 tái lập, B011 mới, B019 mới) |
...
| FR-15 | 15 | 15 | 8 | 7 | 0 | 4 (B009, B010, B017, B018) |
| FR-02 (mobile) | 10 | 9 | 4 | 5 | 1 | 3 (B001 tái lập, B011, B019) |
...
3. **B017 (Critical/High)** — Sửa 1 sản phẩm trong Admin làm hiển thị sai TOÀN BỘ danh sách sản phẩm (đổi tên hàng loạt + dữ liệu cũ) cho đến khi tải lại trang — vi phạm trực tiếp yêu cầu "chỉ sản phẩm được sửa thay đổi".
...
8. **B009 / B010 / B018 (Medium)** — FR-15 hầu như không có validate nào ở tầng dữ liệu: chấp nhận giá 0/âm/rỗng, tên chỉ khoảng trắng, tên vượt quá 255 ký tự không bị cắt/từ chối.

## Kết luận

Sau khi áp dụng Domain Testing và Boundary Value Analysis một cách có hệ thống trên cả 4 feature (web, mobile), tổng cộng 65/66 test case đã được thực thi trên UI thật, phát hiện **16 bug độc lập** với bằng chứng cụ thể (screenshot + đối chiếu code). Các lỗi tập trung vào 3 nhóm chính: **(1) sai công thức/logic nghiệp vụ** (B006, B007, B013 — nhóm nghiêm trọng nhất vì ảnh hưởng trực tiếp đến tiền), **(2) thiếu validate dữ liệu đầu vào** (B009, B010, B018 — điển hình của việc backend "tin tưởng mù quáng" dữ liệu client gửi lên), và **(3) lỗi hiển thị/đồng bộ trạng thái phía client** (B003, B011, B017 — dữ liệu backend đúng nhưng UI hiển thị sai hoặc gây hiểu lầm). Việc đối chiếu chéo giữa web và mobile (Feature D) cũng chứng minh được rằng phần lớn bug nằm ở **backend dùng chung** chứ không phải lỗi ngẫu nhiên của 1 client — một phát hiện có giá trị về mặt phương pháp luận kiểm thử cross-platform.
