...
| FR15-DT-04 | Tên chứa HTML/XSS | name=`<b>Sony</b>`, price=`1000000`, Lưu | Lưu an toàn, hiển thị dạng text (không render/không alert) | EP-04 | Hiển thị **nguyên văn** `<b>Sony</b>` (thấy rõ dấu ngoặc nhọn), không render thành chữ đậm, không có alert — an toàn | **Pass** | — |
...
| FR15-BV-05 | name = chuỗi 256 ký tự, price hợp lệ, Lưu | P-07 | Từ chối / cắt còn 255 (vượt giới hạn) | Tạo thành công KHÔNG cắt bớt — kiểm tra lại vẫn nguyên 256 ký tự. Vi phạm giới hạn tối đa 255 ký tự (spec R1) | **Fail** | B015 |
...
- Số boundary: 2 (price > 0, name.length ≤ 255) — Số TC: 7 (5 BVA + 2 robust), đã chạy 7/7 — 5 Pass, 2 Fail (BV-01 trùng B009, BV-05 → B015 mới)
...
| FR15-DT-08 | Sửa 1 sản phẩm làm hiển thị sai toàn bộ danh sách + dữ liệu cũ cho đến khi F5 | B014 | `FR-15_bugs/B014-1.png` → `B014-4.png` |
| FR15-BV-05 | Tên 256 ký tự vẫn được lưu nguyên, không cắt/từ chối | B015 | `FR-15_bugs/B015.png` |

### Metrics — FR-15

| Designed | Executed | Pass | Fail | Not run | Bugs |
|----------|----------|------|------|---------|------|
| 15 (8 Domain + 5 BVA + 2 robust) | 15 | 8 | 7 | 0 | 4 (B009, B010, B014, B015) |
...
|-------|------|-------------------|----------------|--------|-----------|--------|
| FR02M-DT-01 | cross-check | Đăng nhập đúng `test@eshop.com`/`Test1234!` | Thành công, vào trang chủ | Đăng nhập thành công | **Pass** | — |
| FR02M-DT-02 | cross-check | Sai mật khẩu 1 lần | Từ chối; app hiện thông báo lỗi | Từ chối, hiện "Đăng nhập thất bại. Vui lòng kiểm tra lại." | **Pass** | — |
...
|-------|-------------------|----------|----------------|--------------------------|-----------------|-----------|--------|
| FR02M-BV-01 | Sai mật khẩu lần 1 | attempts=1 | Chưa khóa | Chưa khóa — HTTP 401 tương đương (từ chối chung) | Web: chưa khóa (Pass) | Khớp | — |
| FR02M-BV-02 | Sai mật khẩu lần 2 | attempts=2 | Chưa khóa (theo spec) | Chưa khóa (chưa test độc lập trên mobile, suy ra từ cùng backend đã xác nhận qua web) | Web: chưa khóa (Pass) | Khớp (suy luận) | — |
| FR02M-BV-03 | Sai mật khẩu lần 3 | attempts=3 (on) | Khóa (theo spec) | **Bị khóa đúng sau lần thứ 3** | Web: bị khóa ở lần 3 (Fail — B001) | **Khớp — tái lập y hệt** | B001 |
| FR02M-BV-04 | Sau khi bị khóa, đợi đúng 3 phút rồi đăng nhập lại bằng mật khẩu ĐÚNG | thời gian khóa (đo thực tế) | Theo spec: đã phải mở khóa từ giây thứ 30, tức từ lâu trước mốc 3 phút | **Đăng nhập lại thành công sau đúng 3 phút** — khớp chính xác hằng số `180000`ms, KHÔNG mở khóa sớm hơn ở giây 30 như spec | Web: mở khóa sau ~3 phút, vẫn khóa ở 30s (Fail — B002) | **Khớp — tái lập y hệt** | B002 |

**Biên bổ sung riêng mobile:** thời điểm text thông báo đổi từ "sai mật khẩu" sang "bị khóa" — kỳ vọng đổi tại đúng lần sai chạm ngưỡng; **xác nhận thực tế: không bao giờ đổi** (1 câu chung "Đăng nhập thất bại. Vui lòng kiểm tra lại." xuyên suốt, kể cả khi đã bị khóa).

**Quan sát tích cực (khác biệt so với web, không phải bug):** tiêu đề màn hình mobile hiển thị đúng **"Đăng Nhập"** — KHÔNG mắc lỗi hiện "Đăng Ký" như bug B012 ở web (`Login.jsx:24`). Đây là bằng chứng cho thấy bug B012 chỉ tồn tại ở tầng UI web, không phải lỗi chung của cả hệ thống — minh họa tốt cho việc so sánh chéo giữa 2 client giúp khoanh vùng chính xác bug nằm ở đâu (client cụ thể vs backend dùng chung).

---

## 3. Test Execution — FR-02 Mobile

**Thiết bị:** iPhone thật (Expo Go) · **Ngày:** 2026-07-09 · **Đối chiếu:** đã biết trước response backend từ Feature A (cùng 1 backend, không cần mở song song DevTools cho mọi case).

> Actual chi tiết từng TC đã điền trực tiếp vào bảng Domain (Bước 6) và BVA cross-check ở trên. Bảng dưới tóm tắt các TC Fail.

| TC-ID | Mô tả lỗi | Bug-ID | Screenshot |
|-------|-----------|--------|------------|
| FR02M-DT-03, FR02M-BV-03 | Tài khoản bị khóa sau đúng 3 lần sai — tái lập B001 qua mobile (cùng backend) | B001 | dùng chung ảnh `FR-02_bugs/B001.png` |
| FR02M-BV-04 | Thời gian khóa thực tế ~3 phút (không phải 30s) — tái lập B002 qua mobile | B002 | dùng chung ảnh `FR-02_bugs/B002-*.png` |
| FR02M-DT-04, FR02M-DT-05 | App không phân biệt "sai mật khẩu" và "bị khóa" — thông báo giống hệt | B011 | `FR-02-mobile_bugs/B011.png` |
| FR02M-DT-06 | Không chặn submit khi để trống email/mật khẩu, thiếu dấu `*` bắt buộc | B016 | `FR-02-mobile_bugs/B016.png` |

### Bảng cross-check với Feature A

| Hành vi | Web (Feature A) | Mobile | Nhất quán? |
|---|---|---|---|
| Số lần sai đến khi khóa | 3 lần (sớm hơn 1 lần so với thiết kế đúng — B001) | 3 lần — **đã tự đo lại, khớp y hệt** | ✅ Có — tái lập y hệt, xác nhận bug nằm ở backend dùng chung, không phải riêng 1 client |
| Thời gian khóa đo được | ~3 phút (đã đo chính xác — B002) | **Đã tự đo lại: đăng nhập thành công đúng sau 3 phút, không mở khóa sớm ở giây 30 như spec** | ✅ Có — tái lập y hệt, xác nhận B002 cũng là lỗi backend dùng chung |
| Thông báo khi bị khóa | "Đăng nhập thất bại. Vui lòng kiểm tra lại." — không phân biệt (B003) | Y hệt — không phân biệt (B011) | ✅ Có — cùng pattern lỗi (catch nuốt `data.error`) lặp lại độc lập ở cả 2 codebase |

### Metrics — FR-02 Mobile

| Designed | Executed | Pass | Fail | Not run | Bugs |
|----------|----------|------|------|---------|------|
| 10 (6 Domain + 4 BVA cross-check) | 10 | 4 | 6 | 0 | 4 (B001 tái lập, B002 tái lập, B011 mới, B016 mới) |
...
| Kịch bản / lỗi bị bỏ sót | Root cause | Bài học & khắc phục |
|--------------------------|------------|---------------------|
| AI ban đầu (khi đọc code `handleLogin`) mặc định app hiển thị đúng `data.error` mà backend trả về — pattern phổ biến trong code React Native. Chỉ khi được dán đúng đoạn `catch` (`App.js:204-206`) mới phát hiện được nó **nuốt bỏ** `data.error` để thay bằng câu chung, y hệt bug ở web. | AI suy diễn theo pattern "code React Native thường hiển thị lỗi backend" thay vì đọc kỹ TỪNG DÒNG của đúng hàm `handleLogin` — đây là gap "đọc code hời hợt", khác với gap "suy diễn thiếu dữ kiện" ở FR-02 web hay FR-09 (nơi AI đơn giản là chưa có đủ thông tin để suy ra bug). | Với các hàm xử lý lỗi (`catch`, `.catch()`), phải yêu cầu AI trích dẫn NGUYÊN VĂN từng dòng thay vì tóm tắt hành vi — bug ẩn thường nằm ở chi tiết nhỏ (1 dòng `catch` thay message) mà bản tóm tắt bỏ qua. |
| Ban đầu AI đề xuất **bỏ qua** BV-04 (đo lại thời gian khóa trên mobile) vì đã có bằng chứng rất mạnh từ Feature A (web) — quyết định hợp lý về thời gian, nhưng làm giảm độ "độc lập" của bằng chứng cross-platform nếu dừng lại ở đó. Sau khi sinh viên tự làm thêm test này, kết quả xác nhận thêm B002 (không phải chỉ B001) cũng tái lập y hệt qua mobile — cho thấy việc "bỏ qua vì đã chắc chắn" đôi khi khiến bỏ lỡ một xác nhận có giá trị mà chi phí thực hiện thực ra không lớn. | AI đánh giá độ ưu tiên dựa trên mức độ CHẮC CHẮN của bằng chứng gốc (đã rất mạnh từ web) mà chưa cân nhắc đủ rằng chi phí xác nhận thêm ở mobile là thấp (chỉ cần đợi 3 phút một lần), trong khi giá trị tăng thêm cho báo cáo (cross-check đầy đủ 2 bug thay vì 1) lại đáng kể. | Khi cân nhắc bỏ qua 1 test vì "đã có bằng chứng đủ mạnh", nên so sánh cụ thể CHI PHÍ thực hiện thêm (ở đây rất thấp) với GIÁ TRỊ tăng thêm cho tính đầy đủ của báo cáo, thay vì mặc định bỏ qua chỉ vì đã có 1 bằng chứng khác. |
| AI không tự phát hiện được sự khác biệt CLIENT-SPECIFIC quan trọng (mobile dùng `secureTextEntry` đúng cách — không mắc bug B004 như web; và mobile KHÔNG có kiểm tra rỗng trước submit — khác web có `required`) cho đến khi được yêu cầu đọc lại đúng đoạn JSX của form. | AI mặc định 2 client "chắc hành xử giống nhau" vì cùng chia sẻ backend, bỏ qua rằng phần UI/validate là code RIÊNG BIỆT của từng client, không kế thừa từ nhau. | Khi test cross-platform, phải đọc riêng code UI của TỪNG client (không suy từ client kia) — phần chia sẻ chỉ là backend, còn validate/hiển thị là độc lập hoàn toàn. |

### 2. Cải tiến prompt

1. Prompt cũ: "kiểm tra xem app mobile hiển thị lỗi gì khi bị khóa". → Prompt cải thiện: "dán nguyên văn hàm `handleLogin` (cả try và catch) và hỏi: dòng nào xử lý response lỗi, giá trị `data.error` có được dùng ở đâu không, hay bị bỏ qua?"
2. Prompt cũ: "sinh đủ bộ BVA cross-check giống hệt Feature A". → Prompt cải thiện: "sinh BVA cross-check nhưng đánh dấu rõ mức ưu tiên: điểm nào bắt buộc đo lại (vd điểm biên chưa từng qua thực nghiệm), điểm nào có thể suy luận từ bằng chứng đã có (đã đo rất chắc ở client khác, cùng backend)."
...
|---------|----------|----------|------|------|---------|------|
| FR-02 (web) | 21 | 21 | 13 | 8 | 0 | 6 (B001–B005, B012) |
| FR-09 | 20 | 20 | 13 | 7 | 0 | 4 (B006–B008, B013) |
| FR-15 | 15 | 15 | 8 | 7 | 0 | 4 (B009, B010, B014, B015) |
| FR-02 (mobile) | 10 | 10 | 4 | 6 | 0 | 4 (B001 tái lập, B002 tái lập, B011, B016) |
| **Total** | **66** | **66** | **38** | **28** | **0** | **16 bug ID duy nhất** |

*(B001 và B002 được đếm ở cả FR-02 web lẫn FR-02 Mobile vì cùng 2 bug backend tái lập qua 2 client (đã tự đo lại độc lập trên mobile, không chỉ suy luận) — tổng số bug ID DUY NHẤT toàn bài vẫn là 16, không phải 6+4+4+4=18, do B001/B002 không tính là bug mới.)*

## Các bug quan trọng nhất

1. **B013 (Critical)** — Ô tổng tiền thanh toán chỉnh sửa tự do, kết hợp bug công thức coupon (B007) khiến backend lưu đơn hàng 350.000.000 ₫ từ giỏ hàng thật chỉ đáng 350.000 ₫ — lỗ hổng tài chính nghiêm trọng nhất toàn bài, có bằng chứng đơn hàng thật.
2. **B007 (Critical)** — Công thức tính giảm giá phần trăm sai (`total × (1 − value)` thay vì `total × value / 100`), khiến "tiết kiệm" âm và thành tiền tăng gấp ~10 lần đơn gốc ở mọi mức tiền đã test.
3. **B014 (Critical/High)** — Sửa 1 sản phẩm trong Admin làm hiển thị sai TOÀN BỘ danh sách sản phẩm (đổi tên hàng loạt + dữ liệu cũ) cho đến khi tải lại trang — vi phạm trực tiếp yêu cầu "chỉ sản phẩm được sửa thay đổi".
4. **B001 (High)** — Bộ đếm đăng nhập sai tăng nhanh hơn thiết kế, khóa tài khoản sớm hơn 1 lần — tái lập nhất quán trên cả web và mobile, xác nhận lỗi nằm ở backend dùng chung.
5. **B002 (Medium)** — Thời gian khóa tài khoản thực tế ~3 phút thay vì 30 giây theo spec — tái lập nhất quán trên cả web và mobile.
6. **B006 (Medium)** — Đơn hàng đúng bằng ngưỡng tối thiểu coupon bị từ chối sai (off-by-one `>` thay vì `>=`) — xác nhận là lỗi hệ thống, áp dụng cho mọi loại coupon (SAV10 và BIGBUY).
7. **B003 / B011 (Medium)** — Cả web lẫn mobile đều không phân biệt được thông báo "sai mật khẩu" và "tài khoản đang bị khóa" — người dùng bị khóa không biết lý do thật.
8. **B009 / B010 / B015 (Medium)** — FR-15 hầu như không có validate nào ở tầng dữ liệu: chấp nhận giá 0/âm/rỗng, tên chỉ khoảng trắng, tên vượt quá 255 ký tự không bị cắt/từ chối.

## Kết luận

Sau khi áp dụng Domain Testing và Boundary Value Analysis một cách có hệ thống trên cả 4 feature (web, mobile), tổng cộng 66/66 test case đã được thực thi trên UI thật (100%), phát hiện **16 bug độc lập** với bằng chứng cụ thể (screenshot + đối chiếu code). Các lỗi tập trung vào 3 nhóm chính: **(1) sai công thức/logic nghiệp vụ** (B006, B007, B013 — nhóm nghiêm trọng nhất vì ảnh hưởng trực tiếp đến tiền), **(2) thiếu validate dữ liệu đầu vào** (B009, B010, B015 — điển hình của việc backend "tin tưởng mù quáng" dữ liệu client gửi lên), và **(3) lỗi hiển thị/đồng bộ trạng thái phía client** (B003, B011, B014 — dữ liệu backend đúng nhưng UI hiển thị sai hoặc gây hiểu lầm). Việc đối chiếu chéo giữa web và mobile (Feature D) cũng chứng minh được rằng phần lớn bug nằm ở **backend dùng chung** chứ không phải lỗi ngẫu nhiên của 1 client — một phát hiện có giá trị về mặt phương pháp luận kiểm thử cross-platform.
