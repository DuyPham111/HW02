...
| FR02M-BV-04 | Đợi ~30s rồi đăng nhập đúng | thời gian khóa=30s (on) | Mở khóa, đăng nhập được (theo spec) | *Không test lại — đã có bằng chứng chắc chắn từ Feature A (B002: khóa thực tế ~3 phút), bỏ qua để tiết kiệm thời gian* | Web: vẫn khóa ở 30s (Fail — B002) | Not run (mobile) | — |
...
|---|---|---|---|
| Số lần sai đến khi khóa | 3 lần (sớm hơn 1 lần so với thiết kế đúng — B001) | 3 lần | ✅ Có — tái lập y hệt, xác nhận bug nằm ở backend dùng chung, không phải riêng 1 client |
| Thời gian khóa đo được | ~3 phút (đã đo chính xác — B002) | Không đo lại (bỏ qua để tiết kiệm thời gian, cùng backend nên suy luận ra kết quả tương tự) | (giả định) Có — chưa tự đo lại độc lập trên mobile |
...
| Designed | Executed | Pass | Fail | Not run | Bugs |
|----------|----------|------|------|---------|------|
| 10 (6 Domain + 4 BVA cross-check) | 9 | 4 | 5 | 1 (BV-04, bỏ qua có chủ đích) | 3 (B001 tái lập, B011 mới, B016 mới) |
...
| AI thiết kế BVA-04 (đo lại thời gian khóa 30s trên mobile) nhưng khi thực thi, cả AI và sinh viên đều quyết định **bỏ qua** không đo lại — quyết định hợp lý về thời gian nhưng làm giảm độ "độc lập" của bằng chứng cross-platform (kết quả mobile về thời gian khóa chỉ là suy luận, chưa tự đo). | AI không có cơ chế phân biệt rạch ròi giữa "test cần thiết để CONFIRM lần đầu" và "test lặp lại chỉ để CROSS-CHECK khi bằng chứng gốc đã rất chắc chắn" — nên đưa ra y hệt độ ưu tiên cho mọi BVA point dù có mức độ rủi ro/lợi ích khác nhau. | Khi 1 phát hiện đã có bằng chứng rất mạnh từ 1 client, các TC cross-check tương tự ở client khác nên được đánh giá độ ưu tiên THẤP HƠN (đủ dùng để xác nhận nhanh 1-2 điểm mấu chốt, không cần lặp lại toàn bộ chuỗi đo đạc tốn thời gian). |
...
| FR-02 (mobile) | 10 | 9 | 4 | 5 | 1 | 3 (B001 tái lập, B011, B016) |
| **Total** | **66** | **65** | **38** | **27** | **1** | **16 bug ID duy nhất** |

*(B001 được đếm ở cả FR-02 web lẫn FR-02 Mobile vì cùng 1 bug backend tái lập qua 2 client — tổng số bug ID DUY NHẤT toàn bài là 16, không phải 6+4+4+3=17, do B001 không lặp lại.)*
...
5. **B002 (Medium)** — Thời gian khóa tài khoản thực tế ~3 phút thay vì 30 giây theo spec.
